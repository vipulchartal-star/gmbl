...
PALETTES="
Bull Market|52 124 196 231
Green Candle|22 34 46 231
Alt Season|28 40 46 231
Open Interest|23 35 48 231
Midnight Orderbook|17 27 39 231
Meme Rally|53 129 201 16
Breakout Signal|58 178 226 16
Liquidation Heat|88 202 208 16
"
...
draw_screen() {
  ...
  COLOR_VALUES="$(printf '%s\n' "$pair" | awk -F'|' '{ print $2 }')"
  BG_LOW="$(printf '%s\n' "$COLOR_VALUES" | awk '{ print $1 }')"
  BG_MID="$(printf '%s\n' "$COLOR_VALUES" | awk '{ print $2 }')"
  BG_HIGH="$(printf '%s\n' "$COLOR_VALUES" | awk '{ print $3 }')"
  FG="$(printf '%s\n' "$COLOR_VALUES" | awk '{ print $4 }')"
  ...
  printf '                                                                                \n'
  printf '                         market: %-18s palette: %-2s                           \n' \
    "$MARKET_NAME" "$PALETTE_INDEX"
  printf '                                                                                \n'
  printf '                               bg: %-3s fg: %-3s                               \n' \
    "$BG" "$FG"
  printf '                                                                                \n'
